...
const MaskedHeading = ({
  ...
  settingsRef.current = { fillScale, parallax, drift, brightness, saturation, hueRotate, sepia, grayscale, textScale };
  ...
  const place = useCallback(() => {
    ...
    let filterStr = `brightness(${s.brightness}) saturate(${s.saturation})`;
    if (s.grayscale) filterStr += ' grayscale(1)';
    if (s.sepia > 0) filterStr += ` sepia(${s.sepia})`;
    if (s.hueRotate !== 0) filterStr += ` hue-rotate(${s.hueRotate}deg)`;
